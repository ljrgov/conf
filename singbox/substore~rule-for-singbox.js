// https://git.repcz.link/raw.githubusercontent.com/ljrgov/conf/refs/heads/main/singbox/substore~rule-for-singbox.js#type=组合订阅&name=sublink&outbound=🕳ℹ️🌏 手动选择🕳ℹ️🇭🇰 游戏节点🏷ℹ️^.*🇭🇰.*游戏.*$🕳ℹ️🇯🇵 游戏节点🏷ℹ️^.*🇯🇵.*游戏.*$🕳ℹ️🇭🇰 香港节点🏷ℹ️^(?=.*(?:🇭🇰|香港|HK|Hong Kong))(?!(?:.*Premium|.*游戏|.*tg)).*$🕳ℹ️🇨🇳 台湾节点🏷ℹ️^(?=.*(?:🇨🇳|🇹🇼|台湾|TW|Taiwan))(?!(?:.*Premium|.*游戏|.*tg)).*$🕳ℹ️🇯🇵 日本节点🏷ℹ️^(?=.*(?:🇯🇵|日本|JP|Tokyo|Japan))(?!(?:.*Premium|.*游戏|.*tg)).*$🕳ℹ️🇺🇸 美国节点🏷ℹ️^(?=.*(?:🇺🇸|US|United States))(?!(?:.*bv|.*Premium|.*游戏|.*nx)).*$🕳ℹ️🇸🇬 新加坡节点🏷ℹ️^(?=.*(?:🇸🇬|新加坡|SG|Singapore))(?!(?:.*Premium|.*游戏|.*tg)).*$
