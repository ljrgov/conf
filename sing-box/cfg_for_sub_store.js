...
async function execute() {
    try {
        ...
        // --- 处理 bridge / android_user / ebpf 布尔参数 (默认不传或不为 true 则视为 false) ---
        ...
        if (config.inbounds && Array.isArray(config.inbounds)) {
            ...
            config.inbounds = config.inbounds.filter(inbound => {
                // 1. 如果开启了 ebpf (ebpf=true)，强制干掉 tun 和 tproxy
                if (isEbpfEnabled) {
                    if (inbound.type === "tun" || inbound.type === "tproxy") return false;
                } else {
                    // 2. 如果没开启 ebpf (不填或为 false)，直接移除 ebpf 入站
        ...
        if (mirror) {
            ...
            configString = configString.replace(githubRegex, (match, domain, offset) => {
                // charBefore === '/': 防止 URL 前面紧邻的斜杠导致误判成"已加前缀"
                // alreadyPrefixed: 防止本来就带 mirror 前缀的 URL 被重复加前缀
