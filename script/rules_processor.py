...
_RE_KNOWN_PREFIX = re.compile(
    ...
    r"DST-PORT|SRC-PORT|"
    r"PROCESS(?:-NAME|-PATH(?:-WILDCARD|-REGEX)?)?|PN|"
...
_RE_PROCESS_PREFIX = re.compile(
    r"^(PROCESS(?:-NAME)?|PN|pkg),", re.IGNORECASE
)
    ...
    with open(src_path, encoding="utf-8", errors="replace") as f:
        for raw in f:
            ...
            if _RE_KNOWN_PREFIX.match(line):
                ...
                # 需要 no-resolve 的类型 (增加 IP-SUFFIX/GEOIP 等的覆盖)
                if p in ("IP-CIDR", "IP-CIDR6", "IP-ASN", "GEOIP", "SRC-IP", "IP-SUFFIX",
                         "DST-PORT", "DEST-PORT", "SRC-PORT"):
                    # 避免重复叠加 no-resolve
...
def main():
    ...
    # ── Surge（6+ 兼容5）────────────────────────────────────────
# ── Surge（6+ 兼容5）────────────────────────────────────────
    # 支持类型：域名、IP、ASN、GEOIP、SRC-IP、端口、进程、URL-REGEX、OR、AND、NOT
    SURGE_ALLOWED_PREFIXES = {
        "DOMAIN", "DOMAIN-SUFFIX", "DOMAIN-KEYWORD", "DOMAIN-WILDCARD",
        "IP-CIDR", "IP-CIDR6", "IP-ASN", "DEST-PORT",
        "SRC-PORT",
    ...
    for rule in std_rules:
        ...
        if prefix in ("PROCESS-PATH", "PROCESS-PATH-WILDCARD"):
            # Surge 无 PROCESS-PATH，降级为 PROCESS-NAME
            value = rule.split(",", 1)[1]
            surge_res.append(f"PROCESS-NAME,{value}")
        
        elif prefix in ("OR", "AND", "NOT"):
            # 针对逻辑规则内部的 IP/ASN/Port 相关项补回 no-resolve
            restored = re.sub(
                r"(IP-CIDR6?|IP-ASN|GEOIP|SRC-IP|DST-PORT|SRC-PORT),([^,)]+)",
                r"\1,\2,no-resolve",
                rule,
                flags=re.IGNORECASE,
            )
...
# ── Mihomo（1.19.20+）────────────────────────────────────────
    ...
    for rule in std_rules:
        ...
        if prefix in ("OR", "AND", "NOT"):
            ...
            # 注意：Mihomo 的 PROCESS-PATH 建议保持原始路径格式
    ...
    for rule in std_rules:
        ...
        # ── 逻辑规则收集 ──
        if upper.startswith(("OR,", "AND,")):
            mode = "and" if upper.startswith("AND,") else "or"
        ...
        if p == "DOMAIN":
            ...
            pass  # sing-box 规则集不支持 process_name_regex，忽略
        elif p == "IP-CIDR":
            ip_all.append(v)
        elif p == "IP-CIDR6":
            ip_all.append(v)
