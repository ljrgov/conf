...
if (idx == 3) {
  ...
  const filePaths = await DocumentPicker.openFiles();  // 支持选择多个文件
  folderPath = filePaths[0].substring(0, filePaths[0].lastIndexOf('/'));
  files = filePaths.map(path => path.substring(path.lastIndexOf('/') + 1));
} else if (idx == 1) {
  let url;
  let name;
  if (fromUrlScheme) {
...
if (!checkUpdate && !fromUrlScheme) {
  alert = new Alert();
  let upErrk = report.fail.length > 0 ? `❌ 更新失败: ${report.fail.join(', ')}` : '';
  let noUrlCount = report.noUrl > 0 ? `🈚️ 无订阅链接: ${report.noUrl}` : '';
  alert.title = '处理完成';
  alert.message = `成功: ${report.success}\n${upErrk}\n${noUrlCount}`;
  alert.addCancelAction('关闭');
  await alert.presentAlert();
}
